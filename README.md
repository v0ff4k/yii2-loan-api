...
### Затраяенное время:
1. докер подборка рабочего окружения: 3 часа
2. Код: 2 часа (в сумме)
3. Тесты: 1 час.


### TODO
    - Много лишнего из стандартной установки Yii2  
    - Логика в контроллере вместо сервисного слоя. Контроллер разросся (около 150 строк с двумя экшенами и общим методом) — при появлении сервисов его можно сократить до тонкой обёртки над сервисами.
    - Дублирование валидации — ручная проверка полей в контроллере при уже заданных правилах в модели (нет использования load() + validate()).